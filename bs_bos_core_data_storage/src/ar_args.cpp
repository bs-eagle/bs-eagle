...
namespace blue_sky
  ...
  ar_args::ar_args(const std::string &tname,
                   itype itype_,
                   void *tarray,
...
//			larray = tlarray;
  ...
  ar_args::ar_args(const ar_args &a)
  {
    ni = a.ni;
    nj = a.nj;
    nk = a.nk;
    ...
    if (a.array && n_size)
      {
        ...
        //array = a.array;
				if (arr_type == FPOINT_T) {
					array = (void*)(new t_float[n_size.data ()]);
					memcpy (array, a.array, n_size * sizeof (t_float));
				}
  ...
  ar_args::ar_args(itype itype_, void *tarray, /*const sp_item_array_t &tarray,*/ const std::string &tname)
  {
    i1 = 0;
    i2 = 0;
    j1 = 0;
    ...
    if (tarray)
    {
        ...
        if (arr_type == FPOINT_T)
				...
				if (array == 0) {
					...
					if (arr_type == FPOINT_T)
						((t_float*)array)[0] = *((t_float*)tarray);
					else
...
//		larray.push_back(array);
  ...
  const std::string &ar_args::get_name() const
    {
      //typename ar_args::index_t
      //ar_args::get_name () const {
      return name;
    }

	
  /*const typename ar_args::sp_item_array_t &*/
  void *ar_args::get_array()
  {
    return array;
  }

//	
//	typename ar_args::t_float &ar_args::get (unsigned i) {
//		return array->operator [](i);
//	}

	
  ar_args &ar_args::operator=(const ar_args &a)
  {
    if (this == &a)
      return *this;
		...
		if (a.array)
		{
			...
			if (array == 0) {
				...
				if (arr_type == FPOINT_T)
					((t_float*)array)[0] = *((t_float*)a.array);
				else
  ...
  ar_args::itype ar_args::get_type () const
    {
      return arr_type;
    }

	
  int ar_args::operator==(const std::string &tname) const
    {
  ...
  bool ar_args::operator<(const ar_args &arg) const
    {
      return (name < arg.name);
    }

	
  int ar_args::allocate()
  {
    ...
    if (arr_type == FPOINT_T)
      {
        array = (void *)(new t_float[nb]);
        memset (array, 0, sizeof (t_float) * nb);
      }
    else if (arr_type == INT_T)
      {

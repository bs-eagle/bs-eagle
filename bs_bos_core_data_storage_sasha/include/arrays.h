...
extern const char* array_name_i[];
extern const char* array_name_fp[];
extern const char* array_name_comp[];

BS_API_PLUGIN const std::map< std::string, int >& array_initv_i();
BS_API_PLUGIN const std::map< std::string, float >& array_initv_fp();
BS_API_PLUGIN const std::map< std::string, float >& array_initv_c();

BS_API_PLUGIN const std::map< std::string, blue_sky::pool::array_size >& array_sizes_fp();
BS_API_PLUGIN const std::map< std::string, blue_sky::pool::array_size >& array_sizes_i();
//BS_API_PLUGIN const std::map< std::string, blue_sky::pool::array_size >& array_sizes_c();

// syntactic sugar for reading elements from maps above
template< class map_t >
const typename map_t::mapped_type& at(const map_t& container, const typename map_t::key_type& key) {
	typename map_t::const_iterator p = container.find(key);
	if(p != container.end())
		return p->second;
	else
		throw std::out_of_range("Invalid array name passed as a key");
}

} } 	// namespace blue_sky { pool

...
#define A_GET_VALUE(a,n)     \
	((a.get_type () == ar_args<strategy_t>::FPOINT_T) ? (((typename strategy_t::fp_type_t *)(a.get_array ()))[n]) \
																				 : (((int   *)(a.get_array ()))[n]))

#define A_SET_VALUE(l,n,value)  \
	((l.get_type () == ar_args<strategy_t>::FPOINT_T) ?  (((typename strategy_t::fp_type_t *)(l.get_array ()))[n] = (typename strategy_t::fp_type_t)(value)) \
  ...
  */
  //template<typename T>
	...
	int all_trigon_operations(T(*m_func)(T),
                            ar_args<strategy_t> &ar, ar_stack < ar_args<strategy_t> > &st, bool check = false,
														typename strategy_t::fp_type_t val = 0)
  {
  ...
  {
		return all_trigon_operations <strategy_t, double> (tan,ar,st,true,(((typename strategy_t::fp_type_t)3.141592654) / 2));
  }
  ...
  template <typename strategy_t>
	int all_simple_operations (typename strategy_t::fp_type_t(*math_operation)(const typename strategy_t::fp_type_t&,const typename strategy_t::fp_type_t&),
                             ar_args<strategy_t> &ar, ar_stack < ar_args<strategy_t> > &st,
														 bool check = false, typename strategy_t::fp_type_t val = 0)
  {
  ...
  {
		return all_simple_operations <strategy_t> (add_operation<typename strategy_t::fp_type_t>,ar,st);
  }

  template<typename strategy_t>
  int ar_operation_minus (ar_args<strategy_t> &ar, ar_stack < ar_args<strategy_t> > &st)
  {
    return all_simple_operations <strategy_t> (sub_operation<typename strategy_t::fp_type_t>,ar,st);
  }

  template<typename strategy_t>
  int ar_operation_mult (ar_args<strategy_t> &ar, ar_stack < ar_args<strategy_t> > &st)
  {
    return all_simple_operations <strategy_t> (mul_operation<typename strategy_t::fp_type_t>,ar,st);
  }

  template<typename strategy_t>
  int ar_operation_dev (ar_args<strategy_t> &ar, ar_stack < ar_args<strategy_t> > &st)
  {
    return all_simple_operations <strategy_t> (dev_operation<typename strategy_t::fp_type_t>,ar,st,true,0);
  }
...
#define GET_MIN(A,B) ((A)<(B)?(A):(B))
  ...
  {
		return all_simple_operations <strategy_t> (operation_max<typename strategy_t::fp_type_t>,ar,st);
  }

  template<typename strategy_t>
  int ar_operation_min (ar_args<strategy_t> &ar, ar_stack < ar_args<strategy_t> > &st)
  {
    return all_simple_operations <strategy_t> (operation_min<typename strategy_t::fp_type_t>,ar,st);
  }

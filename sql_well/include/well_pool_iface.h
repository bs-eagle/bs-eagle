...
public:
	 ...
	 * @return 0 if success
	 */
	virtual int open_db (const std::string &file, bool skip_postproc = false) = 0;
	...
	virtual int insert_or_update (
		const std::string &select_sql, const std::string &insert_sql, const std::string &update_sql
	) = 0;
	virtual spv_double get_table (
		const std::string &table_name, boost::python::list &table_columns, const std::string &filter
	) = 0;

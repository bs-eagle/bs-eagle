    ...
    gmres_solver::init_prop ()
      {
        ...
        if (max_iters_idx < 0)
          max_iters_idx = prop->add_property_i (200, std::string ("maxiters"), std::string ("Maximum allowed number of iterations"));
        ...
        m_idx = prop->get_index_i (std::string ("m_size"));
        if ( m_idx < 0)
          m_idx = prop->add_property_i (30, std::string ("m_size"), std::string ("Number of vectors used for ortoganalization"));
